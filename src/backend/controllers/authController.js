...
exports.login = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [user.id, `User ${user.email} logged in`, ip]
    );
...
// ✅ Use middleware to verify cookie token for the next functions
...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (
        username, email, password, display_name, phone, role_id,
        member_since, is_active, is_deleted,
        date_created, date_last_modified, last_modified_by
      ) VALUES (
        pgp_sym_encrypt($1, $8),
...
exports.getUserById = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO audit_log (user_id, action, description, ip_address)
       VALUES ($1, 'VIEW_USER', $2, $3)`,
      [decoded.id, `Viewed user ${userId}`, req.headers['x-forwarded-for'] || req.socket.remoteAddress]
    );
...
exports.getAllUsers = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO audit_log (user_id, action, description, ip_address, timestamp)
       VALUES ($1, 'VIEW_USERS', 'Viewed all users', $2, NOW())`,
      [decoded.id, req.headers['x-forwarded-for'] || req.socket.remoteAddress]
    );
...
exports.resetUserPassword = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [decoded.id, `Admin reset password for user ${userId}`, req.headers['x-forwarded-for'] || req.socket.remoteAddress]
    );
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [decoded.id, `Soft-deleted user ${userId}`, req.headers['x-forwarded-for'] || req.socket.remoteAddress]
    );
